...
// ─── Full auth CSS — injected directly so Register page works even when
//     LoginPage has never been mounted (e.g. Landing → Get Access → Register).
//     Same style-id "auth-styles" is used by LoginPage so the tag is only
//     added once whichever page mounts first.
const AUTH_CSS = `
...
/* Register-specific: taller panel to fit 5 fields without overflow */
...
export default function RegisterPage({ onVerify, onLogin, onBack }) {
  ...
  const validate = () => {
    if (!form.username.trim() || !form.email.trim() || !form.password) return 'All required fields must be filled.';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate to OTP verification page — do NOT auto-login yet
      onVerify?.(data.pending_id, data.email);
    } catch {
      ...
      <div className="auth-brand">
        ...
        <div className="auth-brand-header">
          ...
          <h2 className="auth-brand-tagline">
            Command Officer // <br />Operator Registration
          </h2>

          <p className="auth-brand-sub">
            Create your authorized operator account to access the full
            Sentinel AI surveillance command platform.
          </p>
        </div>
        ...
        <div className="auth-status-badges">
          {[
            ...
            'Secure Session Authentication (SHA-256 / JWT)',
            'Role-Based Command Clearance Level 2',
          ].map(s => (
        ...
        <div className="auth-brand-footer">
          ...
          <span>SECURITY ENROLLMENT GATEWAY</span>
        </div>
      </div>

      {/* ── Right form panel ── */}
      <div className="auth-form-panel auth-form-panel-register">
        <button id="register-back-btn" className="auth-back-link" onClick={onBack}>
          ← Back
        </button>

        <h1 className="auth-form-title">Create Account</h1>
        <p className="auth-form-sub">Fill in the details to register as an operator</p>
        ...
        <form onSubmit={handleSubmit} style={{ width: '100%' }}>
          ...
          <div className="auth-field">
            <label className="auth-label" htmlFor="reg-fullname">Full Name (optional)</label>
            <input
              id="reg-fullname"
              className="auth-input"
              type="text"
              placeholder="Maj. John Doe"
        ...
        <div className="auth-switch">
          ...
          <button id="register-go-login-btn" className="auth-switch-link" onClick={onLogin}>
            Sign In
          </button>
        </div>
      </div>
